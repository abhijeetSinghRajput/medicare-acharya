                    ...
                    if (i < pathnames.length - 1) {
                      return (
                        <DropdownMenuItem key={name} style={{ padding: 0 }}>
                          <Link className="w-full px-2 py-1.5" to={route}>
                            {name || "Medicare Acharya"}
                          </Link>
                        </DropdownMenuItem>
                      );
                    }
                  ...
                  <div key={i} className="hidden sm:flex items-center gap-2">
                    <BreadcrumbItem>
                      <BreadcrumbLink asChild>
                        <Link to={route}>{name || "Medicare Acharya"}</Link>
                      </BreadcrumbLink>
                    </BreadcrumbItem>
                    <BreadcrumbSeparator />
                  </div>
                    ...
                    <DropdownMenuContent className="space-y-1" align="start">
                      {navLinks.map(({ title, url, icon: Icon }) => {
                        return (
                          <DropdownMenuItem
                            key={title}
                            className={`${isActivePath(url) && "bg-accent"}`}
                            style={{ padding: 0 }}
                          >
                            <Link
                              className="flex items-center w-full gap-2 px-2 py-1.5"
                              to={url}
                            >
                              <Icon size={16} />
                              {title}
